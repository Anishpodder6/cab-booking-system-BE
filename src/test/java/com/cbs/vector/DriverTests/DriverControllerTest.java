...
import com.cbs.vector.controller.DriverController;
import com.cbs.vector.dto.DriverResponseDTO;
import com.cbs.vector.dto.DriverUpdateDTO;
...
class DriverControllerTest {
    ...
    @Test
    @DisplayName("GET /profile/{id} - Should return OK when driver service returns null unexpectedly")
    void getDriverAllDetails_shouldReturnOkWhenServiceReturnsNull() throws Exception {
        UUID driverId = UUID.randomUUID();
        when(driverService.findUserById(driverId)).thenReturn(null);

        mockMvc.perform(get("/api/drivers/profile/" + driverId))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").doesNotExist());

        verify(driverService).findUserById(driverId);
    }
