...
import com.cbs.vector.controller.PaymentController;
...
class PaymentControllerTest {
    ...
    void testGetReceipt_Found() throws IOException {
        // *** CHANGE HERE: getReceiptByPaymentId to getReceiptByRideId ***
        when(paymentService.getReceiptByRideId(rideId)).thenReturn(Optional.of(payment));
        when(paymentService.generatePaymentReceipt(payment)).thenReturn(new byte[]{1, 2, 3});

        ResponseEntity<?> response = paymentController.getReceipt(rideId);
        ...
        // *** CHANGE HERE: getReceiptByPaymentId to getReceiptByRideId ***
        verify(paymentService, times(1)).getReceiptByRideId(rideId);
    ...
    void testGetReceipt_NotFound() {
        ...
        // *** CHANGE HERE: getReceiptByPaymentId to getReceiptByRideId ***
        when(paymentService.getReceiptByRideId(notFoundId)).thenReturn(Optional.empty());

        ResponseEntity<?> response = paymentController.getReceipt(notFoundId);

        assertEquals(HttpStatus.NOT_FOUND, response.getStatusCode());
        assertTrue(response.getBody().toString().contains("Receipt for ride ID"));
        // *** CHANGE HERE: getReceiptByPaymentId to getReceiptByRideId ***
        verify(paymentService, times(1)).getReceiptByRideId(notFoundId);
        try {
